...
- `POST /admin/backups` must not generate the archive on the request path; it writes
  ...
  payload `{ "backup_id": "<numeric id string>" }`.
...
- Backup and export JSON must redact keys/columns containing password/token/secret/code,
  including `hashed_password`, `token_hash`, `refresh_token_hash`, `code_hash`,
  `idempotency_key`, and job payload secrets.
...
- Bad: export includes `background_jobs.payload.secret`, password-reset
  `background_jobs.idempotency_key`, or `users.hashed_password` raw value.
