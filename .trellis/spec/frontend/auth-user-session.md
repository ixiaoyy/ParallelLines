...
- Topbar/profile/security growth displays use API-provided level/points/experience fields and must not
  duplicate backend level threshold rules.
...
| Public profile/current user includes growth fields | UI renders level/points/experience as metadata and does not infer permissions from it. |
